...
import { Sparkles, Clock, Gift, PartyPopper, Star, Timer } from 'lucide-react';
...
export const CountdownBox: React.FC<CountdownBoxProps> = ({
  ...
  subtitle = "Teachers' Day Grand Ceremony & Exclusive Gifts Reveal",
  isDark = false,
  onExploreGifts,
  className = '',
}) => {
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 flex flex-col items-center text-center gap-4">
        ...
        >
          <Gift size={28} className={isDark ? 'text-[#ffe088]' : 'text-[#241a00]'} />
        </div>
          ...
          >
            ...
            <div className="text-center sm:text-left">
              ...
              <p className="text-xs" style={{ color: isDark ? 'rgba(255,255,255,0.55)' : '#7b757f' }}>
                Celebration & exclusive gifts are unlocked now.
              </p>
            </div>
            {onExploreGifts && (
              <button
                onClick={onExploreGifts}
                className="mt-1 sm:mt-0 sm:ml-auto flex-shrink-0 px-4 py-2 rounded-xl text-xs font-bold uppercase tracking-wider cursor-pointer transition-all hover:scale-105 active:scale-95 shadow-md"
                style={{
                  background: 'linear-gradient(135deg, #ffe088, #fcd34d)',
                  color: '#241a00',
                  boxShadow: '0 4px 15px rgba(255,224,136,0.35)',
                }}
              >
                Explore
              </button>
        ...
        {/* Explore Gifts Button (pre-reveal) */}
        {!timeLeft.isExpired && onExploreGifts && (
          <button
            onClick={onExploreGifts}
            className="flex items-center gap-2 px-5 py-2 rounded-xl text-xs font-bold uppercase tracking-wider cursor-pointer transition-all hover:scale-105 active:scale-95"
            style={{
              background: isDark ? 'rgba(255,224,136,0.12)' : 'rgba(254,214,91,0.20)',
              border: `1px solid ${isDark ? 'rgba(255,224,136,0.3)' : '#ddc98a'}`,
              color: isDark ? '#ffe088' : '#735c00',
            }}
          >
            <Timer size={13} />
